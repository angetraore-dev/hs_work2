...
if(isset($_GET['id'])){
    ...
    $numeroSite = $row['numeroSite'];
    $zone = $row['zone'];
    $recouvreur = $row['recouvreur'];
    $solde = $row['solde'];
    $comment = $row['comment'];
...
if(isset($_GET['id'])){
    ...
    $query = "select * from clients where id=?";
    $stmt = $sql->prepare($query);
    $stmt->bindParam(1, $filedata['id'], PDO::PARAM_STR);
    $stmt->execute();
    $client = $stmt->fetch();
}
        ...
        <div class="collapse show" id="client">
          <ul class="nav ms-4">
            ...
            <li class="nav-item active">
              <a class="nav-link active" href="all-client.php">
                        ...
                        <div class="card">
                            ...
                            <div class="card-header">
                                    <span class="float-right">
                                        <a href="all-client.php" class="btn btn-primary btn-sm">Retour</a>
                                    <?php if (isset($_GET['clients'])) { ?>
                                        <a href="edit-client?id=<?php echo $filedata['id']; ?>&client=<?php echo $filedata['id']; ?>" class="btn btn-primary btn-sm">Editer le profil</a>
                                    <?php } else { ?>
                                        <a href="edit-client?id=<?php echo $filedata['id']; ?>" class="btn btn-primary btn-sm">Editer le profil</a>
                                    <?php } ?>
                                </span>

                            </div>
                            <div class="table-responsive py-4">
                            <h1 class="px-4 py-2 text-center">Infos client</h1>
                            <table class="table align-items-center table-bordered" id="">
                                <tr>
                                    <th>Code de connection</th>
                                    <td><?php echo $code ?></td>
                                    <td><button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#exampleModal">Modifier</button></td>
                                </tr>
                                <tr>
                                    <th>Nom </th>
                                    <td><?php echo $name ?></td>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Moddification</h5>
